...
5. Here, $\Psi^F(P_{U,X}) = $ \Sexpr{round(Psi.F1, 2)}. In words: the counterfactual expected test score would be \Sexpr{round(Psi.F1, 2)} points higher if all students got 8 hours of sleep than if all students got less than 8 hours of sleep the night before their statistics test, with no loss to follow up.
\end{solution}
...
5. $\Psi^F(P_{U,X}) = $ \Sexpr{round(Psi.F2, 2)}. In other words, the counterfactual expected test score would be \Sexpr{round(Psi.F2, 2)} points higher if all students got 8 hours of sleep for all 4 nights than if all students got less than 8 hours of sleep for all 4 nights before their statistics test.
\end{solution}
...
5. The true dose-response curve's intercept is \Sexpr{round(MSM$coefficients[[1]], 3)} and slope is \Sexpr{round(MSM$coefficients[[2]], 3)}. This means that, if we believe this MSM is correct, cumulative days with 8 more hours of sleep has a positive linear effect on the mean counterfactual statistics test scores. For one more night of 8 or more hours of sleep, students' statistics test scores increase by \Sexpr{round(MSM$coefficients[[2]], 3)} points, on average.
...
Thus, projecting the causal dose-response curve onto the working MSM yields coefficients of $\beta_0 = $ \Sexpr{round(MSM$coefficients[[1]], 3)} and $\beta_1= $ \Sexpr{round(MSM$coefficients[[2]], 3)}. In other words, summarizing the true causal dose-response curve with a linear working MSM suggests that cumulative sleep has a positive linear effect on mean statistics exam score.
...
Now, the true dose-response curve's slope is \Sexpr{round(TrueMSMbeta1_wts, 3)}. This means that, if we believe this MSM is correct, cumulative days with 8 more hours of sleep has a positive linear effect on the mean counterfactual statistics test scores. For one more night of 8 or more hours of sleep, students' statistics test scores increase by \Sexpr{round(TrueMSMbeta1_wts, 3)} points, on average.
...
# plot the test score as a function of cumulative sleep (unweighted)
plot(sum.abar, EY.abar,
     main='Expected counterfactual test score as a function of cumulative sleep',
     xlab='Days with 8 or more hours of sleep',
     ylab='Expected counterfactual test score',
...
# add the true causal curve to the graph (in blue).
abline(MSM, col='blue')
...
5. $\Psi^F(P_{U,X}) = $ \Sexpr{round(Psi.F3, 2)}. The counterfactual probability of getting sick would be \Sexpr{round(Psi.F3, 2)*100}\% higher if all students got 8 or more hours of sleep for 2 nights before their statistics test than if all students got less than 8 hours of sleep for 2 nights before their statistics test.
...
5. $\Psi^F(P_{U,X}) = $ \Sexpr{round(Psi.F4, 2)}. The counterfactual probability of getting sick would be \Sexpr{-1*round(Psi.F4, 2)*100}\% lower if all students got 8 or more hours of sleep for 2 nights before their statistics test than if all students got less than 8 hours of sleep for 2 nights before their statistics test, forcing all students to take the exam.
