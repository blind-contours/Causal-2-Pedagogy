...
\[
\Psi^F(P_{U,X}) = E_{U,X}[Y_{\bar{a}(2)=1}] = P_{U,X}(Y_{\bar{a}(2)=1} = 1) = \Sexpr{round(Psi.F0, 4)}
\]
...
We are interested in the difference in the expected test score if all students got 8 or more hours of sleep for all 4 nights before the test versus if all students got less than 8 hours of sleep for all 4 nights before the test is \Sexpr{round(Psi.F2, 4)} points.
\[
\Psi^F(P_{U,X}) = E_{U,X}[Y_{\bar{a}(4)=1} - Y_{\bar{a}(4)=0}] = \Sexpr{round(Psi.F2, 4)}
\]
